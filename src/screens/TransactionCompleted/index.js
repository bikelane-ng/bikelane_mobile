...
const TransactionCompleted = ({ tranx, status, creditAccount, proceed, }) => {
    ...
    return (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.9)', }}>
            ...
            {tranx && <CheckCircle />}
            {tranx && <Text style={{ color: colors.white, marginTop: 30, fontFamily: fonts.medium, fontSize: 16, }}>{'Payment Successfuly'}</Text>}
            ...
            {tranx && <View style={{ padding: 50, width: '100%', position: 'absolute', bottom: 70, }}>
                <Button
                    onPress={() => proceed()}
                    BtnText={"Complete"}
                    BtnTextStyles={{ color: '#ffffff' }}
