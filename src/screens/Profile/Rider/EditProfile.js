...
class EditProfile extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.user.modified &&
      nextProps.user.modified != this.props.user.modified
    ) {
    ...
    if (
      nextProps.user.credentials &&
      nextProps.user.credentials != this.props.user.credentials
    ) {
      this.setState({
        loaded: true,
    ...
    if (
      nextProps.driver.avatar &&
      nextProps.driver.avatar != this.props.driver.avatar
    ) {
      Toast.show({
        type: 'success',
          ...
          <View style={{flex: 1, padding: 30}}>
              ...
              <View>
                <TouchableOpacity onPress={() => this.selectPhoto()}>
                  <Thumbnail
                    ...
                    source={
                      this.props.driver.credentials &&
                      this.props.driver.credentials.avatarPhoto == null
                        ? require('../../../imgs/avatar-profile-picture-3.png')
                        : {
                            uri: `${this.props.driver.credentials &&
                              this.props.driver.credentials.avatarPhoto}`,
                          }
                    }
                  />
                </TouchableOpacity>
                ...
                <View style={{paddingLeft: 20, flex: 1}}>
                  <View style={{flexDirection: 'row'}}>
                    ...
                    <View style={{position: 'absolute', right: 0}}>
                      <Icon
                        style={{color: '#00A87E', fontSize: 20}}
                        type="Ionicons"
            ...
            <View style={{marginTop: 30}}>
              <Button
                ...
                style={[
                  {backgroundColor: '#00A87E', height: 50},
                  {opacity: this.state.isProcessing ? 0.4 : 1},
                ]}
                block>
                <Text style={{color: 'white'}}>
                  {this.state.isProcessing ? 'Processing...' : 'SAVE CHANGES'}
                </Text>
              </Button>
            </View>
          </View>
