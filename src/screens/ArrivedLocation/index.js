...
const ArrivedLocation = ({ rideDetails, arrived, isRideStatusUpdated, error, }) => {
    const { fare, duration, distance, } = rideDetails || {};
    return (
        <>
            {error && <View style={{ backgroundColor: colors.danger, padding: 5, }}>
                <Text style={{ fontFamily: fonts.medium, fontSize: 12, color: colors.white, }}>{error}</Text>
            </View>}
            {rideDetails && <>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 30, paddingTop: 0, }}>
                    <View>
