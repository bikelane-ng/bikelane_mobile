  ...
  return dispatch => {
    Geocoder.from(region.latitude, region.longitude)
      .then(json => {
        ...
        RNGooglePlaces.lookUpPlaceByID(addressComponent.place_id).then(
          results => {
            dispatch({
...
export function getRouteDirections() {
  return (dispatch, store) => {
    if (store().home.inputData.pickUp && store().home.inputData.dropOff) {
      let query = {
        origins:
          store().home.inputData.pickUp.location.latitude +
          ',' +
          store().home.inputData.pickUp.location.longitude,
        destinations:
          store().home.inputData.dropOff.location.latitude +
          ',' +
          store().home.inputData.dropOff.location.longitude,
        mode: 'driving',
        key: 'AIzaSyA-HjztLKyWGOUaIG9Bx_n6Ie_A5p1qMkQ',
      };
      const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${
        query.origins
      }&destination=${query.destinations}&key=${query.key}&mode=${query.mode}`;
      Axios.get(url)
        .then(response => {
          dispatch({
            type: GET_ROUTE_DIRECTIONS,
            payload: polyline.decode(
...
export function getNearbyDrivers() {
  return (dispatch, store) => {
    ...
    FakeDrivers(userLocation.latitude, userLocation.longitude).forEach(
      drivers => {
    ...
    var nearestDriver = nearbyDrivers.reduce((prev, curr) =>
...
export const requestRide = details => (
  console.log(details),
  {
    [RSAA]: {
        ...
        payload: (action, state, response) =>
          response.json().then(response => {
            store.getState().socket.emit('RideStatus', {
              ...
              location: location,
              transactionDetails:
                status == rideStats.COMPLETED ? response.data : undefined,
            });
            return {
              response,
              status,
